    ...
    stage('Build') {
      steps {
        script {
          ...
          for (def option in ["px4_same70xplained-v1", "px4_stm32f4discovery", "px4_cannode-v1", "px4_esc-v1", "px4_nucleoF767ZI-v1", "px4_s2740vc-v1"]) {
            def node_name = "${option}"
          ...
          // posix_sitl
          for (def option in ["px4_sitl_default", "px4_sitl_rtps"]) {
            def node_name = "${option}"
          ...
          for (def option in ["emlid_navio2_cross", "parrot_bebop_default"]) {
            def node_name = "${option}"
          ...
          // GCC7 posix
          for (def option in ["px4_sitl_default"]) {
            def node_name = "${option}"
